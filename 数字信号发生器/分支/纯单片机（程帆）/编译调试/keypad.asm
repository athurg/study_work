...
; This file generated Mon Dec 22 20:33:56 2008
;--------------------------------------------------------
	.module keypad
...
_key_make:
	...
	ar1 = 0x01
	mov	r2,dpl
;	keypad.c:14: struct keypad keytable[]={
	...
	mov	(_key_make_keytable_1_1 + 0x0019),#0x00
	mov	(_key_make_keytable_1_1 + 0x001a),#0xD7
	mov	(_key_make_keytable_1_1 + 0x001b),#0x0B
	mov	(_key_make_keytable_1_1 + 0x001c),#0xB7
	mov	(_key_make_keytable_1_1 + 0x001d),#0x0C
	mov	(_key_make_keytable_1_1 + 0x001e),#0x77
	mov	(_key_make_keytable_1_1 + 0x001f),#0x18
	mov	(_key_make_keytable_1_1 + 0x0020),#0x00
	mov	(_key_make_keytable_1_1 + 0x0021),#0x46
;	keypad.c:26: for(i=16;i>=0;i--){
	mov	r3,#0x10
00103$:
	mov	a,r3
	jb	acc.7,00106$
;	keypad.c:27: if(keytable[i].index==key)	return keytable[i].value;
	mov	a,r3
	add	a,r3
	mov	r4,a
...
;------------------------------------------------------------
;	keypad.c:32: char key_scan(void)
;	-----------------------------------------
;	 function key_scan
;	-----------------------------------------
_key_scan:
;	keypad.c:46: unsigned char line=0x08;			//行扫描起始码(00001000B)
	mov	r2,#0x08
;	keypad.c:48: while(line){
00103$:
	mov	a,r2
	jz	00105$
;	keypad.c:49: KEYPAD_DB=~line;
	mov	a,r2
	cpl	a
	mov	_P1,a
;	keypad.c:50: if((KEYPAD_DB>>4)!=0x0f) break;
	mov	a,_P1
	swap	a
	anl	a,#0x0f
	mov	r3,a
...
;	keypad.c:51: line/=2;
	...
	sjmp	00103$
00105$:
;	keypad.c:53: line=KEYPAD_DB;
	mov	r2,_P1
;	keypad.c:54: KEYPAD_DB=0xf0; //重新初始化P1口，自己的事情自己搞完
	mov	_P1,#0xF0
;	keypad.c:56: return line;
	mov	dpl,r2
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
